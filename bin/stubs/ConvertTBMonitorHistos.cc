...
ConvertTBMonitorHistos::ConvertTBMonitorHistos(string& tb_path, string& client_path, sistrip::Task task) : 
...
ConvertTBMonitorHistos::~ConvertTBMonitorHistos() {
...
bool ConvertTBMonitorHistos::convert() {
...
void ConvertTBMonitorHistos::setRunInfo() {
  ...
  //Get the run number from each file in list and compare...
    ...
    cout << "[ConvertTBMonitorHistos::setRunInfo]: Run number: " << run << endl;
...
std::string ConvertTBMonitorHistos::taskId(sistrip::Task task) {
  ...
  else {cout << "[ConvertTBMonitorHistos::taskId]: Unknown Commissioning task, filling event with ALL TProfile's found in specified files.";}
...
SiStripHistoNamingScheme::HistoTitle ConvertTBMonitorHistos::histoTitle(const string& histo_name) {
  ...
  if (task_ == sistrip::OPTO_SCAN) {
    ...
    else {cout << "[ConvertTBMonitorHistos::histoTitle]: Inconsistency in TBMonitor histogram name for the OPTO_SCAN task. One or more of the strings \"gain\", \"tick\" and \"base\" were not found.";}
  }
  ...
  else {cout << "[ConvertTBMonitorHistos::histoTitle]: Unknown Commissioning task. Setting SiStripHistoNamingScheme::HistoName::granularity_ to \"UNKNOWN GRANULARITY\".";}
...
//-----------------------------------------------------------------------------
 ...
 void ConvertTBMonitorHistos::lldPedestals(TProfile& module, vector<TProfile>& llds) {
